...
@app.get("/")
def root():
    return {"app": "OptiMax API", "version": "3.1.0", "total_stocks": len(SHARIAH_STOCKS), "market_open": is_market_open(), "message": "Advanced 10-Indicator Analysis (FMP API)"}
...
def get_top_opportunities():
    ...
    if claude_client and len(top_50) > 0:
        ...
        final_top_20 = top_50[:20]
    result = {"updated_at": datetime.now(pytz.timezone('US/Eastern')).isoformat(), "market_open": is_market_open(), "total_analyzed": len(all_scores), "sp500_performance": round(sp500_return, 2), "top_opportunities": final_top_20, "analysis_method": "Financial Modeling Prep API"}
...
def market_status():
    et = pytz.timezone('US/Eastern')
    now_et = datetime.now(et)
    return {"market_open": is_market_open(), "current_time_et": now_et.strftime("%Y-%m-%d %H:%M:%S ET (%A)"), "market_hours": "9:30 AM - 4:00 PM ET (Mon-Fri)", "total_stocks": len(SHARIAH_STOCKS), "cache_duration": f"{CACHE_DURATION} seconds", "claude_enabled": claude_client is not None, "fmp_enabled": bool(FMP_API_KEY), "analysis_version": "3.1.0 - FMP API", "indicators": ["SMA-200 Filter", "RSI + Volume", "MACD Crossover", "Bollinger Bands", "Money Flow Index (MFI)", "ADX (Trend Strength)", "ATR (Dynamic Stops)", "Rate of Change (ROC)", "Gap Analysis", "Relative Strength"]}
